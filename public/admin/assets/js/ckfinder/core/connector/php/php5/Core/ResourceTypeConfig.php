...
class CKFinder_Connector_Core_ResourceTypeConfig
{
    ...
    public function checkExtension(&$fileName, $renameIfRequired = true)
    {
        if (strpos($fileName, '.') === false) {
            // In this case check if empty extension is allowed (passing null will check for "no_ext")
            return $this->checkSingleExtension(null);
        }
    ...
    public function checkSingleExtension($extension)
    {
        ...
        if ($extension === CKFinder_Connector_Utils_FileSystem::FILENAME_WITHOUT_EXTENSION) {
            return false;
        }
        ...
        if (empty($extension)) {
            $extension = CKFinder_Connector_Utils_FileSystem::FILENAME_WITHOUT_EXTENSION;
        }
